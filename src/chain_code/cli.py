...
@click.option("--steps", default=30, show_default=True, help="Maximum planning/execution steps.")
@click.option("--confirm/--no-confirm", default=False, show_default=True, help="Require confirmation before each command.")
def agent(objective: str, steps: int, confirm: bool) -> None:
    """Autonomous mode: let the model plan and run shell commands."""
    cfg = load_config()
    approve = bool(confirm and cfg.get("approve_shell", True))
    history = ""
...
"""
        ...
        if not data:
            # permissive fallback: treat model output as direct shell command
            command = raw.strip()
            if not command:
                console.print("[red]Model returned empty output.[/red]")
                raise SystemExit(1)
            console.print("[yellow]Non-JSON output detected; executing raw output as shell command.[/yellow]")
